...
const fetchAccounts = async () => {
  const res = await fetch("http://localhost:4000/api/settings/cloud-accounts");
...
export default function SettingsPage() {
  ...
  const addAccountMutation = useMutation({
    ...
    onSuccess: (res) => {
      setNotification({ message: res.message || "Account connected successfully!", type: "success" });
    ...
    onError: () => {
      setNotification({ message: "Failed to connect account. Please check credentials.", type: "error" });
    }
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`http://localhost:4000/api/settings/cloud-accounts/${id}`, {
        method: "DELETE"
      });
    ...
    onSuccess: () => {
      setNotification({ message: "Cloud account disconnected.", type: "success" });
      queryClient.invalidateQueries({ queryKey: ["cloud-accounts"] });
    }
  });
  ...
  const handleTestConnection = async (provider: string, id?: string) => {
    ...
    try {
      const res = await fetch("http://localhost:4000/api/settings/test-connection", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setNotification({ message: "Connection test failed.", type: "error" });
    } finally {
      setTestingId(null);
    }
  };
  ...
  return (
    <div className="space-y-xl pb-24">
      ...
      {/* Alert / Toast */}
      {notification && (
        <div className={`p-4 rounded-xl flex justify-between items-center ${notification.type === "success" ? "bg-emerald-500/10 border border-emerald-500/30 text-emerald-300" : "bg-error/10 border border-error/30 text-error"}`}>
      ...
      <div className="space-y-4">
        ...
        {isLoading ? (
          <div className="p-8 text-center text-on-surface-variant">Loading connected accounts...</div>
        ) : (
              ...
              <div key={acc.id} className="bg-surface-container/50 p-5 rounded-xl border border-white/10 hover:border-primary/40 transition-all flex flex-col justify-between space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-2 border-t border-white/5">
                  <button
                    ...
                    className="px-3 py-1.5 text-xs font-medium rounded-lg bg-surface-variant hover:bg-primary/20 text-primary transition-colors flex items-center gap-1 cursor-pointer"
                  >
                    <span className="material-symbols-outlined text-sm">{testingId === acc.id ? "sync" : "build"}</span>
                    {testingId === acc.id ? "Testing..." : "Test Credentials"}
                  </button>
                  <button
                    onClick={() => deleteMutation.mutate(acc.id)}
                    className="px-3 py-1.5 text-xs font-medium rounded-lg bg-error/10 hover:bg-error/20 text-error transition-colors flex items-center gap-1 cursor-pointer"
                  >
      ...
      <div className="bg-surface-container-low p-6 rounded-2xl border border-white/10 space-y-6">
          ...
          <div className="flex gap-2">
            {(["AWS", "GCP", "AZURE"] as const).map((tab) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg font-label-sm font-semibold transition-all cursor-pointer ${
                  activeTab === tab
        ...
        <form onSubmit={handleSaveAccount} className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-surface-variant/50 border border-white/10 rounded-xl px-4 py-2.5 font-body-md text-on-surface focus:outline-none focus:border-primary"
              />
            </div>
            <div>
              ...
              {authMethod === "role" ? (
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-surface-variant/50 border border-white/10 rounded-xl px-4 py-2.5 font-mono text-sm text-on-surface focus:outline-none focus:border-primary"
                  />
          ...
          {activeTab === "AZURE" && (
            <div className="p-4 bg-surface-variant/30 rounded-xl text-on-surface-variant font-body-md">
              Azure Cost Management API integration is ready for tenant connection. Enter your Subscription ID above.
            </div>
          )}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
            <button
              ...
              className="px-5 py-2.5 rounded-xl border border-white/10 font-label-sm font-semibold text-on-surface hover:bg-surface-variant transition-colors cursor-pointer"
            >
              Test API Connection
            </button>
            <button
              ...
              className="px-6 py-2.5 rounded-xl bg-primary text-on-primary font-label-sm font-semibold hover:brightness-110 shadow-lg shadow-primary/25 transition-all cursor-pointer"
            >
