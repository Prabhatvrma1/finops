...
const navLinks = [
  ...
  { href: "/infrastructure", icon: "insights", label: "Infrastructure" },
  { href: "/settings", icon: "cloud", label: "Cloud Providers" },
];
    ...
    <div className="bg-background text-on-surface font-body-md antialiased ambient-bg min-h-screen flex overflow-hidden">
      ...
      <nav className="hidden md:flex flex-col h-screen py-lg bg-surface-container/40 backdrop-blur-xl border-r border-white/10 shadow-xl docked left-0 w-64 sticky z-40 shrink-0">
        ...
        <div className="flex-1 px-sm space-y-sm overflow-y-auto">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`${pathname === link.href ? 'bg-primary-container text-on-primary-container' : 'text-on-surface-variant hover:text-on-surface hover:bg-secondary-container/20'} rounded-lg px-4 py-3 flex items-center gap-3 transition-all cursor-pointer duration-300`}
            >
        ...
        <div className="px-lg mt-auto pt-lg border-t border-white/5 space-y-sm">
          <button className="w-full bg-primary-container text-on-primary-container font-label-sm py-2 rounded-lg hover:brightness-110 transition-all shadow-[0_0_12px_rgba(15,164,175,0.3)] cursor-pointer active:scale-95">
            Upgrade Plan
          </button>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden" onClick={() => setMobileMenuOpen(false)}>
          ...
          <nav
            className="absolute left-0 top-0 h-full w-64 bg-surface-container/95 backdrop-blur-xl border-r border-white/10 shadow-xl py-lg flex flex-col"
          ...
          >
            <div className="px-lg mb-xl flex items-center justify-between">
              ...
              <button onClick={() => setMobileMenuOpen(false)} className="text-on-surface-variant hover:text-on-surface cursor-pointer">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        {/* TopNavBar */}
        <header className="bg-surface/40 backdrop-blur-md font-headline-lg border-b border-white/10 shadow-sm docked full-width top-0 sticky z-40 flex justify-between items-center w-full px-lg py-sm max-w-full shrink-0">
          <div className="md:hidden flex items-center gap-sm">
            <button onClick={() => setMobileMenuOpen(true)} className="text-on-surface-variant hover:text-on-surface p-1 cursor-pointer">
          ...
          <div className="flex items-center gap-md">
            ...
            <div className="w-8 h-8 rounded-full overflow-hidden border border-outline-variant/50 ml-sm cursor-pointer hover:border-primary transition-colors bg-primary/20 flex items-center justify-center">
        ...
        <div className="flex-1 overflow-y-auto p-margin-mobile md:p-margin-desktop scroll-smooth">
          <div className="max-w-7xl mx-auto space-y-xl pb-xl">
            ...
            {/* Footer */}
            <footer className="flex justify-between items-center px-4 md:px-margin-desktop py-lg w-full mt-xl bg-surface-container-lowest border-t border-outline-variant/50 relative bottom-0 rounded-xl flex-col md:flex-row gap-4">
              ...
              <div className="flex gap-md font-label-sm text-on-surface-variant">
                <Link className="hover:text-primary transition-colors underline-offset-4 hover:underline" href="/dashboard">Dashboard</Link>
                <Link className="hover:text-primary transition-colors underline-offset-4 hover:underline" href="/infrastructure">Infrastructure</Link>
                <Link className="hover:text-primary transition-colors underline-offset-4 hover:underline" href="/settings">Settings</Link>
              </div>
            </footer>
          </div>
        </div>
